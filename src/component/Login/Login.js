...
import loginImg from './Mobile login-cuate.svg'
import useToken from './useToken';
import resetImg from './Forgot password-bro.svg'
import regImage from './Mobile login-amico.svg'
...
const Login = () => {
    ...
    return (
        <div className='p-6 m-3 flex w-full items-center flex-col md:flex-row'>
            <div className='w-[50%] hidden md:block'>
                {
                    (registerForm && !ResetPass) &&
                    <div>
                        <img src={loginImg} alt="" className='h-[500px] xl:h-[600px] ' />
                    </div>
                }
                {
                    (!registerForm && !ResetPass) &&
                    <div>
                        <img src={regImage} alt="" className='h-[500px] xl:h-[600px] ' />
                    </div>
                }

                {
                    (!registerForm && ResetPass) &&
                    <div>
                        <img src={resetImg} alt="" className='h-[500px] xl:h-[600px] ' />
                    </div>
                }
            </div>
            <div className=' w-80 mx-auto'>
                <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col'>
                    ...
                    <input type="submit" value={ResetPass ? 'Reset Password' : (registerForm ? 'Login' : 'Register')} className='btn btn-primary  m-2 text-white w-full' />
                    {
                ...
                <div className="divider">OR</div>
                {
                    ...
                    <div>
                        <button
                            ...
                            className='flex items-center btn btn-success text-white gap-3  w-full'
                        >
                            <img src={google} alt="" />
                            <span>
                                Sign {registerForm ? 'in' : 'up'} with Google
                            </span>
                        </button>
                    </div>
                }
